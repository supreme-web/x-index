...
type RouteParams = { niche: string; company: string };
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { niche, company } = await Promise.resolve(params);
  ...
  const alternates: Metadata["alternates"] = {
    ...
    languages: {
      "pt-BR": `https://br.x-index.com/${niche}/${company}`,
      "pt-PT": `https://pt.x-index.com/${niche}/${company}`,
      "en-US": `https://us.x-index.com/${niche}/${company}`,
      "en-GB": `https://uk.x-index.com/${niche}/${company}`,
      "fr-FR": `https://fr.x-index.com/${niche}/${company}`,
      "it-IT": `https://it.x-index.com/${niche}/${company}`,
      "es-ES": `https://es.x-index.com/${niche}/${company}`,
      "x-default": `https://x-index.com/${niche}/${company}`,
    },
  };
        ...
        <div className="p-8 border border-zinc-200 dark:border-zinc-800 rounded-2xl bg-zinc-50 dark:bg-zinc-900 text-left space-y-4 inline-block shadow-sm">
          ...
          <p>
            <span className="font-bold text-blue-600">
              {isBr ? "Empresa:" : "Company:"}
            </span>{" "}
            {companyName}
          </p>
          <p>
            <span className="font-bold text-blue-600">
              {isBr ? "Serviço:" : "Service:"}
            </span>{" "}
            {nicheName}
          </p>
          <p>
